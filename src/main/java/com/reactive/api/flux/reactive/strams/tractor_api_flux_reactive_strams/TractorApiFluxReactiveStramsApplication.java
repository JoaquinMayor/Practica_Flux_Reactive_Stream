...
public class TractorApiFluxReactiveStramsApplication implements CommandLineRunner{ //Se implementa el CommandLineRunner para que se pueda ejecutar por consola
	...
	public void run(String... args) throws Exception {
		...
		Flux<Usuario> nombres = Flux.just("Andres Guzman", "Pedro Fulano", "Diego Sultano","Juan Mengano","Maria Fulana", "Bruce Lee","Bruce Willis")//Creamos observables con el flux y su método just, donde se le pasa la data
		.map(nombre -> new Usuario(nombre.split(" ")[0].toUpperCase(), nombre.split(" ")[1].toUpperCase()))
		.filter(usuario -> usuario.getName().equalsIgnoreCase("Bruce")) //Filtramos por el nombre bruce
		.doOnNext(element ->{
			if(element == null){
				throw new RuntimeException("Nombres no pueden ser vacíos"); //manejo de errores dentro del doOnNext, esto termina el flujo
			}
				System.out.println(element.getName().concat(" ").concat(element.getLastname()));
			})//Esto es lo que va a hacer cuando nos suscribamos, notifica cuando haya llegado un elemento
			.map(usuario ->{
				String nombre = usuario.getName().toLowerCase();
				usuario.setName(nombre);
				return usuario;}); //Retornamos otra instancia del flux con los datos cambiados, el map sirve para generar otra instancia modificada, la posición del map afecta a la ejecución si se pone antes se mostrarán en mayúsculas , si se pone después no, afectando solo a los métodos del subscribe
		...
		//En los subscribe se pueden pasar 3 parámetros, el código que queremos que se ejecute, el código del error y el código de cuando termina de ejecutarse, con el Runnable que es una interface
	}
